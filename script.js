...
function generatePassword() {
  ...
  function promptYN(promptText,defaultValue) {
    ...
    let yn=defaultValue.toLowerCase;

    // Check defaultValue is a valid choice, if not set to y (yes) arbitrarily
    if (defaultValue != 'y' && defaultValue != 'n') {defaultValue='y'};

    // Loop until user submits a valid response or cancels the prompt
    do {
      yn = prompt(promptText,defaultValue).toLowerCase();
  ...
  do {
    ...
    var includeUpperCase = promptYN("Include upper case letters (A-Z)? (y/n)", "y");
    if (includeUpperCase === null) return("");

    var includeLowerCase = promptYN("Include lower case letters (a-z)? (y/n)", "y");
    if (includeLowerCase === null) return("");

    var includeNumbers = promptYN("Include numbers (0-9)? (y/n)", "y");
    if (includeNumbers === null) return("");

    var includeSymbols = promptYN("Include symbols? (y/n)", "y");
